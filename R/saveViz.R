...
#' It supports visualizations generated by `plotVenn()`, `plotUpSet()`, `ggplot2`, or any other plot object that can be rendered using `print()` inside a graphics device.
#' Optionally, the current date (stored in the `today` variable) can be prepended to the filename.
#'
#' @param viz A visualization object typically created by either `plotVenn()` or `plotUpSet()`, but can also be a `ggplot2` plot or any other plot object printable with `print()`.
#' @param output_dir A string specifying the output directory. Defaults to `"."`.
#' @param output_file A string specifying the base filename (without extension). Defaults to `"viz_genomicVenn"`.
#' @param format Output format. One of `"pdf"`, `"png"`, or `"svg"`. Defaults to `"pdf"`.
...
#' @param resolution Resolution in DPI (only used for PNG). Default is 300.
#' @param verbose Logical. If `TRUE`, print a message with the saved path. Default `TRUE`.
#'
#' @return The visualization is saved to a file on disk. Invisibly returns the full path to the saved file.
#' @export
#'
#' @examples
#' \dontrun{
