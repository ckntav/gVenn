...
#' @param verbose Logical. If `TRUE` (default), prints `diagError` and
#'   whether it exceeds the 1e-6 threshold, and names any populated region
#'   the diagram gives no area to. The diagnostics are attached to the
#'   returned object regardless (see `Value`).
#' @param ... Additional arguments passed to \code{\link[eulerr]{plot.euler}}.
#'
#' @return A Venn diagram plot generated by \code{eulerr}, with two
#'   attributes:
#'   \itemize{
#'     \item `"fit_diagnostics"`: a list with `stress`, `diagError`,
#'       `regionError` (see \code{\link[eulerr]{euler}}) and
#'       `undrawnRegions`, the populated regions given no area. `diagError`
#'       is the largest difference, over all regions, between the share of
#'       the diagram's area a region receives and the share its count
#'       requires; Micallef and Rodgers (2014) consider a diagram accurate
#'       when it is at most 1e-6.
#'     \item `"euler_fit"`: the `eulerr` fit itself.
#'   }
#' @export
#'
